...
export function AdminSidebar({ activeItem, setActiveItem, isOpen, onClose }: AdminSidebarProps) {
  return (
    <>
      {/* Sidebar Drawer */}
      <aside className={cn(
        "bg-[#7a1f32] text-white border-r border-[#5a1725] transition-all duration-300 ease-in-out",
              ...
              return (
                <AriaButton
                  ...
                  className={cn(
                    ...
                    isActive 
                      ? "bg-white text-[#7a1f32] shadow-lg" 
                      : "text-white/70 hover:bg-white/10 hover:text-white"
                  )}
                >
                  {isActive && (
                    <div className="absolute left-0 top-0 bottom-0 w-1 bg-[#4b90a1]" />
                  )}
                  <Icon size={18} className={cn(
                    "transition-colors duration-200",
                    isActive ? "text-[#7a1f32]" : "text-white/50 group-hover:text-white"
                  )} />
        ...
        {/* Footer area */}
        <div className="p-4 border-t border-[#5a1725] bg-[#5a1725]/30">
          <div className="flex items-center gap-3 p-2 rounded-xl bg-white/5 border border-white/5">
            <div className="w-10 h-10 rounded-full bg-[#4b90a1] flex items-center justify-center text-white font-bold shadow-inner">
              AD
            </div>
