...
export function FacultyDashboard() {
	...
	const pendingRequests = bookings.filter(
		(b: any) => !["APPROVED", "REJECTED", "CANCELLED"].includes(b.status),
	);
		...
		<div className="space-y-6">
			<div>
				...
				<p className="text-sm text-gray-500">
					Review and manage pending academic and administrative permission
					requests
				</p>
			</div>
								...
								{pendingRequests.length === 0 ? (
									...
									pendingRequests.map((request: any) => (
										<div
											...
											className="p-4 border border-gray-100 rounded-xl bg-white shadow-sm hover:border-accent transition-colors"
										>
											<div className="flex justify-between items-start">
												<div className="flex-1">
													...
													<p className="text-xs text-gray-600 mb-1">
														...
														{request.club?.clubName || "Unknown Club"}
													</p>
													<p className="text-xs text-gray-600 mb-1">
														<span className="font-semibold text-gray-700">
															Venue:
														...
														{request.venue?.name} •{" "}
														{new Date(request.eventStart).toLocaleString()}
													</p>
						...
						<Card className="p-5">
							...
							<div className="space-y-3">
								<p className="text-xs text-gray-500 leading-relaxed">
									Select a request from the list to view full details and
									perform approval or rejection reviews.
								</p>
							</div>
						</Card>
					)}
