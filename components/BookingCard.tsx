    ...
    <Card className="p-4 flex flex-col justify-between border border-gray-100 shadow-sm hover:shadow-md transition-shadow">
      <div>
        <div className="flex justify-between items-start mb-3 gap-2">
          <h3 className="text-sm font-bold text-gray-800 line-clamp-1 flex-1">{booking.title}</h3>
      ...
      <div>
        {showActions && (
          <div className="flex flex-wrap gap-1.5 mt-1.5">
            <Button variant="success" size="sm" onPress={onAccept} className="flex-1 sm:flex-none">Accept</Button>
            <Button variant="danger" size="sm" onPress={onReject} className="flex-1 sm:flex-none">Reject</Button>
          </div>
        )}
